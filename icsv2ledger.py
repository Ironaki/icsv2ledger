...
def parse_args_and_config_file():
    ...
    preparser.add_argument(
        ...
        metavar="STR",
        help=(
            "ledger account used as source" " (default: {0})".format(DEFAULTS.account)
        ),
    )
    preparser.add_argument(
        "--config-file",
    ...
    if args.config_file and (
        "-h" not in remaining_argv and "--help" not in remaining_argv
    ):
        config = configparser.RawConfigParser(DEFAULTS)
        config.read(args.config_file)
        if not config.has_section(args.account):
            print(
                "Config file {0} does not contain section {1}".format(
    ...
    parser.add_argument(
        ...
        default=sys.stdin,
        help=(
            "input filename or stdin in CSV syntax" " (default: {0})".format("stdin")
        ),
    )
    parser.add_argument(
        "outfile",
        ...
        default=sys.stdout,
        help=(
            "output filename or stdout in Ledger syntax"
    ...
    parser.add_argument(
        "--encoding",
        metavar="STR",
        help=("encoding of csv file" " (default: {0})".format(DEFAULTS.encoding)),
    )
    parser.add_argument(
        "--ledger-binary",
        metavar="FILE",
        help=(
            ...
            " (default search order: {0})".format(
                ", ".join(FILE_DEFAULTS.ledger_binary_file)
            )
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "do not prompt if account can be deduced"
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "number of lines to skip from CSV file"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "reverse the order of entries in the CSV file"
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "CSV column number matching date" " (default: {0})".format(DEFAULTS.date)
        ),
    )
    parser.add_argument(
        "--effective-date",
        ...
        type=int,
        help=(
            "CSV column number matching effective date"
    ...
    parser.add_argument(
        ...
        metavar="STR",
        help=(
            "CSV column number matching description"
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "CSV column number matching debit amount"
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "CSV column number matching credit amount"
    ...
    parser.add_argument(
        "--currency",
        metavar="STR",
        help=("the currency of amounts" " (default: {0})".format(DEFAULTS.currency)),
    )
    parser.add_argument(
        "--csv-decimal-comma",
        action="store_true",
        help=(
            "comma as decimal separator in the CSV"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " (default search order: {0})".format(
                ", ".join(FILE_DEFAULTS.accounts_file)
            )
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " (default search order: {0})".format(
                ", ".join(FILE_DEFAULTS.template_file)
            )
        ),
    )
    parser.add_argument(
        ...
        "-t",
        action="store_true",
        help=("prompt for transaction tags" " (default: {0})".format(DEFAULTS.tags)),
    )
    parser.add_argument(
        "--multiline-tags",
        action="store_true",
        help=(
            "format tags on multiple lines"
    ...
    parser.add_argument(
        ...
        help=(
            "delimiter between fields in the csv"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "ask before adding entries to mapping file"
    ...
    if args.ledger_binary is not None:
        ...
        if find_first_file(args.ledger_binary, []) is None:
            ...
            raise FileNotFoundError(
                "Can't find ledger binary at the specified location: {0}".format(
                    args.ledger_binary
                )
            )
    else:
    ...
    if args.ledger_date_format and not args.csv_date_format:
        print(
            "csv_date_format must be set" " if ledger_date_format is defined.",
            file=sys.stderr,
        )
        sys.exit(1)
...
class Entry:
        ...
        if options.ledger_date_format:
            if options.ledger_date_format != options.csv_date_format:
                self.date = datetime.strptime(
                    self.date, options.csv_date_format
            ...
            if options.ledger_date_format:
                if options.ledger_date_format != options.csv_date_format:
                    self.effective_date = datetime.strptime(
                        self.effective_date, options.csv_date_format
                    ).strftime(options.ledger_date_format)
        else:
            self.effective_date = ""
    ...
    def prompt(self):
        ...
        return "{0} {1:<40} {2}".format(
            self.date, self.desc, self.credit if self.credit else "-" + self.debit
        )

    def _build_entry_str(
        self, transaction_index, payee, credit_account, debit_account, tags
    ) -> str:
        """
        ...
        """
        template = (
            self.transaction_template if self.transaction_template else DEFAULT_TEMPLATE
        )
    ...
    def journal_entry(self, transaction_index, payee, debit_account, tags):
        return self._build_entry_str(
            transaction_index, payee, self.credit_account, debit_account, tags
        )

    def transfer_entry(self, transaction_index, payee, account, transfer_to, tags):
        return self._build_entry_str(
            transaction_index, payee, account, transfer_to, tags
        )
...
def from_ledger(ledger_file, ledger_binary_file, command):
    ...
    try:
        ...
        raise FileNotFoundError(
            "The system can't find the following ledger binary: {0}".format(ledger)
        )
        ...
        for row in map_reader:
            if len(row) > 2:
                ...
                tags = [
                    col
                    for col in row[3:]
                    if not col.startswith(("transfer_to", "file"))
                ]
                transfer_to = (
                    row[3].split("=")[1].strip()
                    if "".join(row[3:]).startswith("transfer_to=")
                    else None
                )
                transfer_to_file = (
                    row[4].split("=")[1].strip()
                    if "".join(row[4:]).startswith("file=")
                    else None
                )
                ...
                mappings.append(
                    MappingInfo(
                        pattern, payee, account, tags, transfer_to, transfer_to_file
                    )
                )
    return mappings
...
def main(options):
    ...
    if options.ledger_file:
        possible_accounts = accounts_from_ledger(
            options.ledger_file, options.ledger_binary
        )
    ...
    def get_payee_and_account(entry):
        ...
        if not found or (found and modified):
            ...
            if value.upper().strip() not in ("N", "NO"):
                ...
                mappings.append(
                    MappingInfo(entry.desc, payee, account, tags, None, None)
                )
                append_mapping_file(
                    options.mapping_file, entry.desc, payee, account, tags
                )
    ...
    def process_csv_lines(csv_lines):
        ...
        for i, row in enumerate(bank_reader):
            ...
            if (options.skip_older_than < 0) or (
                entry.days_old <= options.skip_older_than
            ):
                if options.clear_screen:
                    print("\033[2J\033[;H")
                print("\n" + entry.prompt())
                if (
                    options.skip_dupes or options.confirm_dupes
                ...
                while True:
                    ...
                    if options.entry_review:
                        ...
                        print(
                            entry.journal_entry(
                                transaction_index + 1, payee, account, tags
                            )
                        )
                        yn_response = prompt_for_value(
                        ...
                        with open(transfer_to_file, "rb") as f:
                            if f.read(1):
                                with mmap.mmap(
                                    ...
                                    has_entry = (
                                        s.find(bytes(entry.md5sum, "utf-8")) != -1
                                    )
                            else:
                                has_entry = False
